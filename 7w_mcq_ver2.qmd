...
import("news_20210908_20220907.xlsx") -> df
```

```{r}
df %>% names()

df %>% select(일자, 언론사, 제목, 본문, cat = `통합 분류1`, 키워드) -> df
...
```{r}
plot(storage) # 배타성, 지속성, 잔차 등 3개 지표에서 모두 주제의 수가 5개인 모형이 우수하여 5로 지정하기로함. 
```
...
text_gamma %>% 
  arrange(-tGamma2) %>% 
  filter(str_detect(text, "대선")) %>% 
  mutate(text = str_replace_all(text, "대선", "**대선**")) %>% 
  pull(text) %>% 
  head(5)
```
...
prep <- estimateEffect(formula = 1:4 ~ press + s(month, 5),
                       stmobj = meta_fit,
                       metadata = out$meta,
                       uncertainty = "Global")
...
plot.estimateEffect(
  ...
  xlim = c(-.1, .1),
  labeltype = "custom",
  custom.labels = c("주제1", "주제2", "주제3", "주제4")
)
```
...
term_coef_time %>% 
  ...
  filter(str_detect(name, "^1|^2|^3|^4")) %>% 
