...
```{r}
pnko_mfd_df %>% lm(care ~ press1, .) %>% tidy()
```

```{r}
pnko_mfd_df %>% lm(care ~ press1 + positive + negative, .) %>% tidy()
```

```{r}
pnko_mfd_df %>% lm(fair ~ press1, .) %>% tidy()
```

```{r}
pnko_mfd_df %>% lm(fair ~ press1 + positive + negative, .) %>% tidy()
```

```{r}
pnko_mfd_df %>% lm(loyal ~ press1 + positive + negative, .) %>% tidy()

```

```{r}
pnko_mfd_df %>% lm(autho ~ press1, .) %>% tidy()
```

```{r}
pnko_mfd_df %>% lm(autho ~ press1 + press1:negative + press1:positive, .) %>% tidy()
```

```{r}
pnko_mfd_df %>% lm(sanct ~ press1, .) %>% tidy()
```

```{r}
pnko_mfd_df%>% lm(sanct ~ press1 * (positive + negative), .) %>% tidy()
```
